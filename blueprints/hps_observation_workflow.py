"""
    ...
    Model: ext.workflows.observation.HpsObservationWorkflow
...
from ext.workflows.hps_observations import HpsObservationWorkflow

# Need custom decorators
from ext.app.decorators import *
from ext.app.eve_helper import eve_response

OrsWorkflow = Blueprint('HPS Observation Workflow', __name__, )
...
def state(observation_id):
    ...
    wf = HpsObservationWorkflow(object_id=observation_id, user_id=g.user_id)
...
def transition(observation_id, action):
    ...
    wf = HpsObservationWorkflow(object_id=observation_id, user_id=g.user_id, comment=comment)

    # Now just do a

    if wf.get_resource_mapping().get(action, False):
        # result = wf.call(get_actions2().get(action)) #getattr(HpsObservationWorkflow, wf.get_actions2().get(action))()
...
def graphit(observation_id, state):
    # wf = HpsObservationWorkflow(object_id=observation_id, user_id=g.user_id)
    from ext.workflows.hps_observations import WF_HPS_STATES, WF_HPS_TRANSITIONS
    if state in WF_HPS_STATES:
        wf = Dummy()
        import io
