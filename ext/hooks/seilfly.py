...
from ext.scf import ACL_SEILFLY_ORS, ACL_SEILFLY_SU, ACL_SEILFLY_CLUB_DTO
from ext.workflows.seilfly_observations import ObservationWorkflow, get_wf_init, get_acl_init
...
def ors_before_insert_item(item):
    try:
        if 'discipline' in item and item.get('discipline', 0) > 0:
            ...
            persons_dto = ACL_SEILFLY_CLUB_DTO.copy()
    ...
    if updates.get('workflow', {}).get('state', None) is None:
        if original.get('workflow', {}).get('state', 'original') not in ['closed', 'withdrawn']:
            ors_save(
                ...
                event_from='seilfly_observations',
