...
from ext.scf import ACL_SPORTSFLY_ORS, ACL_SPORTSFLY_FTU, ACL_SPORTSFLY_CLUB_OPERATIV_LEDER
from ext.workflows.sportsfly_observations import ObservationWorkflow, get_wf_init, get_acl_init
    ...
    if updates.get('workflow', {}).get('state', None) is None:
        if original.get('workflow', {}).get('state', 'original') not in ['closed', 'withdrawn']:
            ors_save(
                ...
                event_from='sportsfly_observations',
...
def ors_before_post_comments(resource, items):
    if resource == 'sportsfly/observation/comments':
